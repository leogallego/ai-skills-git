...
- Does the plan respect layer boundaries? (Check confirmed conventions for layer names and prohibited cross-layer dependencies)
...
- If the project uses platform abstractions: are new abstraction declarations complete with all required implementations? (Check confirmed conventions)
...
- Is it in the correct source set or directory? (Check confirmed conventions for project's source organization)
- Does the file location follow existing project conventions?
- Does shared/common code avoid platform-specific imports? (Check confirmed conventions for prohibited import patterns)

### Check 5: State Management

If the plan creates or modifies ViewModels or UI state:
- Does the plan specify proper encapsulation of mutable state? (Check confirmed conventions for the project's exposure pattern)
- Does the plan use the project's standard state pattern? (Check confirmed conventions for documented pattern)
...
- Are interfaces named following the project's prefix/suffix convention? (Check confirmed conventions)
- Are key component types named following established patterns? (Check confirmed conventions for component naming rules)

### Check 10: Test Strategy

For each test the plan proposes:
- Is it in the correct test source set for its dependency scope?
- Does it use the correct test framework for its source set? (Check confirmed conventions for documented test framework rules)
- Does the plan specify the correct test setup utilities? (Check confirmed conventions for documented setup patterns)
