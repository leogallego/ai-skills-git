...
1. **Existing pipeline PR**: search PRs closing `#N` — skip ONLY if head branch
   matches `<type>/<N>-*` (pack default) or contains `pipeline-issue`. Manual
   non-pipeline PRs do not cause a skip.
